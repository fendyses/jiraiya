# MyAlumniCard — Post-Mortems
*Failures + lessons for this repo. Append-only.*

---

## 2026-04-20 → 2026-07-21 — Token swap silently killed `tarikh_create` for 3 months

**Detected**: 2026-07-21 by Fendy, ~3 months after onset. Analysed same day.
**Status** (as of 2026-07-23): Historical damage **repaired** — Fendy backfilled on 21 Jul; verified 16,788 records at/after 17 Apr now carry `tarikh_create`, all genuine Firestore `Timestamp`s, zero strings. New-record gap **mitigated and verified working** — Firebase Function `stampTarikhCreate` deployed 22 Jul (CR `CR/7-2026.md`, 22-07-2026); it fills `tarikh_create` from the doc `createTime` when upstream doesn't stamp, and self-limits to docs missing the field so it stops once upstream recovers. Verified 23 Jul: query that returned **0** on 22 Jul now shows **33 stamped since deploy, 9 today**; logs confirm the function firing ("tarikh_create filled in") on live registrations (docs 1174350, 836146) with real `created_at` timestamps and genuine `timestampValue`s. Confirmed it's the function doing the work, not upstream quietly recovering (`register.component.ts:175` `myatp` token unchanged). Records stamped at creation moment (`registered:false`) — correct, since `tarikh_create` means "created" not "registered", matching historical rows. **Upstream root cause still live** — the `myatp` token is unchanged; the proper fix (an `alumnai`-issued register token from Integrasi) remains open, now non-urgent since the function covers the daily gap.
